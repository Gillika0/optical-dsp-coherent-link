"""Streamlit page: coherent optical transmission (PDM QPSK / higher-order QAM).

The sidebar configures the dual-polarisation coherent link; the main pane
shows a live KPI bar (line rates, spectral efficiency, FEC coding gain,
required OSNR) plus the recovered-signal visualizations produced by
``run_link``.
"""
...
def _sidebar() -> tuple[LinkConfig, str]:
    ...
    return cfg, modulation


def _kpi_bar(cfg: LinkConfig, order: int) -> None:
    """Live KPI strip: rates, spectral efficiency, FEC gain and required OSNR."""
    const = get_constellation(cfg.modulation)
    fec_code = resolve_fec(cfg.fec)
    ref_bw = ref_bandwidth_hz(1550.0e-9, 0.1)
    rs = cfg.symbol_rate

    raw = line_rate_gbps(rs, order, npol=2)
    net = net_rate_gbps(raw, fec_code)
    se = spectral_efficiency_bits_s_hz(order, npol=2)

    if fec_code is not None:
        target = _pre_fec_threshold_ber(fec_code, 1e-12)
        gain = fec_coding_gain_db(order, fec_code)
    else:
    ...
    col2.metric("Net line rate", f"{net:.1f} Gbps")
    col3.metric("Spectral efficiency", f"{se:.1f} bit/s/Hz")
    if gain is not None:
        col4.metric("FEC coding gain", f"{gain:.2f} dB")
    else:
        col4.metric("FEC coding gain", "—")
    col5.metric(f"Required OSNR @ {target:.1e}", f"{req_osnr:.2f} dB")
    st.caption(
        f"Rates assume dual polarisation (2 x {const.name.replace('DP-', '')}), "
        f"symbol rate {rs / 1e9:.0f} GBd. Required OSNR is the AWGN limit in "
        "0.1 nm reference bandwidth to reach the pre-FEC threshold above."
    )
...
def _eye_tabs(res: LinkResult) -> None:
    st.header("Eye diagrams")
    st.caption(
        "Pre-DSP eye on the photodiode + TIA output (which can look closed "
        "before chromatic-dispersion compensation) vs the post-DSP eye after "
        "CDC and the matched filter: the opening height is set by the symbol "
        "level spacing, its centring by the (zero) decision threshold, and "
        "flat tops would indicate ADC clipping."
    )
    tab_x, tab_y = st.tabs(["Polarisation X", "Polarisation Y"])
    sps = res.config.sps
    with tab_x:
...
def main() -> None:
    """Streamlit entry point for the coherent transmission page."""
    st.set_page_config(page_title="Coherent Optical Link & DSP Simulator", layout="wide")
    st.title("Coherent Optical Transmission")
    st.caption(
